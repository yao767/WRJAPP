const appProjectName = '护花使者——植保无人机适配系统APP';
const appVersionLabel = '1.0.9+9';
const appContactEmail = '3302875008@qq.com';
const appUpdateManifestUrls = [
  'https://raw.githubusercontent.com/yao767/WRJAPP/main/update.json',
  'https://cdn.jsdelivr.net/gh/yao767/WRJAPP@main/update.json',
];

const appUsageSteps = [
  '1. 登录后在主页面点击“+”添加飞行任务。',
  '2. 配置任务类型、飞行参数与环境因子。',
  '3. 点击“一键建议并设置参数”自动填充推荐参数。',
  '4. 点击“保存当前参数”后，任务会进入主页历史列表。',
  '5. 在设置页可保存全局参数并模拟蓝牙连接与参数下发。',
  '6. 在关于页查看版本信息与联系方式。',
];

const appUsageSyncNote = '使用说明会随着后续开发持续同步更新。';
const appFlightControlNote = '说明：当前飞控协议尚未接入，已预留连接与参数下发接口。';
